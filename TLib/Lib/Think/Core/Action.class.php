...
{//类定义开始
   ...
   /**
    ...
    {
        $this->_dispatch_jump($message,$status,$ajax);
    }
    protected function info($message,$status=1,$ajax=false)
    {
        $this->_dispatch_jump($message,$status,$ajax);
    }
	    protected function info2($message,$poi,$status=false,$ajax=false)
    {
        // 判断是否为AJAX返回
        if($ajax || $this->isAjax()){
			$arrayToJs[0][0] = $poi;
			$arrayToJs[0][1] = $status;
			$arrayToJs[0][2] = $message;
